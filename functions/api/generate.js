export async function onRequestPost(context) {
    try {
        ...
        const finalDallePrompt = `I attest that I own the copyrights to this specific prompt. Please use this prompt exactly as written, without any modifications or additions: A single, vertical artwork in the physical medium of heavy linocut printmaking and thick wax crayon on textured, unpainted off-white cream paper. The visual content, specific shapes, and color palette MUST be entirely driven by these psychological emotions: ${resultJson.english_keywords}. Create a few bold, distinct, abstract or highly stylized primitive symbols that visually translate these exact emotions. The shapes must be defined by VERY THICK, heavy, solid black outlines. Choose an opaque color palette that directly reflects the mood of the keywords (e.g., use dark, muted colors for heavy/melancholic keywords, or vibrant, contrasting colors for energetic/passionate keywords). AVOID defaulting to faces or human figures unless the keywords strongly imply it. AVOID thin scratchy lines, AVOID messy overlapping scribbles, AVOID fine details, and AVOID gradients. Flat 2D composition, highly graphic, borderless.`;
