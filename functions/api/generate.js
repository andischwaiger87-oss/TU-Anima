...
Medium: heavily saturated opaque gouache and tempera on watercolor paper, applied WET and THICK with a loaded round bristle brush. Each individual brushstroke is CLEARLY VISIBLE — distinct directional drag marks, separate stroke tracks, and crisp stroke edges where one brushstroke ends and another begins. Brushwork is gestural and energetic, like an expressionist painter working confidently with a wide brush. Visible pigment buildup at stroke endings, paint pooling and darker accumulated edges where wet strokes overlap, occasional impasto ridges catching imagined light. Some passages use translucent overlapping washes; other passages are densely opaque. The image MUST contain deep saturated darks — near-black, deep indigo, burnt umber, or charcoal — somewhere in the composition, providing strong tonal contrast against the lighter mid-tones and highlights. CRITICAL: this is NOT airbrushed, NOT soft pastel, NOT colored pencil, NOT smooth digital gradient blending. Colors meet with visible brushstroke boundaries — no soft photographic fades within color areas. The whole image is photographed from a 1950s European printed book plate, so a PROMINENTLY VISIBLE offset-print halftone rosette pattern (clearly recognizable CMYK dot grid, visible on every color area) overlays the entire image and the colors are slightly muted and shifted by print aging. Color-field outlines are organic and hand-painted (never vector-clean or geometric), but within each color field the individual brushstrokes remain clearly visible.
